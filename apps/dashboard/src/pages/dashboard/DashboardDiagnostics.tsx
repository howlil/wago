...
export function DashboardDiagnostics({ dashboard }: DashboardDiagnosticsProps) {
  const diagnosticsLayout = dashboard.lastMessage
    ? "grid items-start gap-3 xl:grid-cols-[minmax(0,1.7fr)_minmax(320px,0.8fr)]"
    : "w-full";
  const canOpenSendTool = dashboard.status === "connected";

  return (
    <details className="group mt-4 border-t border-wago-workspace-line pt-2">
      ...
      {!canOpenSendTool ? (
        <div className="mt-1.5 w-full rounded-md border border-wago-workspace-line bg-wago-workspace-strong px-3 py-2.5">
          <strong className="block text-xs font-semibold text-wago-ink">Diagnostics unavailable</strong>
          <p className="mb-0 mt-0.5 max-w-prose text-[11px] leading-4 text-wago-muted">
            Connect WhatsApp before running an outbound delivery diagnostic.
          </p>
        </div>
      ) : (
        <div className={`mt-2 ${diagnosticsLayout}`}>
          <SendMessageCard
