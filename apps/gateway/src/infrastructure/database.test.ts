...
describe("SQLite persistence", () => {
  it("uses WAL mode and applies all schema migrations", () => {
    ...
    expect(
      database.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'webhook_settings'").get(),
    ).toBeDefined();
    expect(
    ...
    expect(
      database.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'recipient_identities'").get(),
    ).toBeDefined();
  });
