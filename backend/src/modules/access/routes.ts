...
appRouter.post("/bootstrap", (req, res) => {
  ...
  if (config.nodeEnv === "production" && !requestHasValidBrowserSession(req)) {
    ...
    return res.status(401).json({
      ...
      message: "Sign in to the Wago dashboard before first pairing.",
    });
  }
...
appRouter.post("/session", browserSignInRateLimit, (req, res) => {
  ...
  if (!config.adminPassword) {
    return res.status(503).json({
      success: false,
      error: "ADMIN_PASSWORD_REQUIRED",
      message: "Configure WAGO_ADMIN_PASSWORD in the deployment before signing in.",
    });
  }
...
appRouter.post("/api-key/rotate", (req, res) => {
  ...
  void recordActivity({
    ...
    description:
      "The machine API key was rotated, other dashboard sessions were revoked, and the current session remains active.",
...
appRouter.post("/session/logout-all", (req, res) => {
  ...
  if (config.nodeEnv === "production" && !requestHasSameOrigin(req)) {
    return res.status(403).json({
      success: false,
      error: "INVALID_SESSION_ORIGIN",
      message: "Dashboard session changes must come from the Wago origin.",
    });
  }
