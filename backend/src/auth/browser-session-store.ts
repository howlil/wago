...
const insertSessionStatement = database.prepare(`
  INSERT INTO browser_sessions (id, token_hash, created_at, last_seen_at, expires_at, revoked_at)
  VALUES (?, ?, ?, ?, ?, NULL)
`);
const readSessionStatement = database.prepare(`
  SELECT id, expires_at, revoked_at FROM browser_sessions WHERE token_hash = ?
`);
const touchSessionStatement = database.prepare(`UPDATE browser_sessions SET last_seen_at = ? WHERE id = ?`);
...
`);
const revokeOtherSessionsStatement = database.prepare(`
  UPDATE browser_sessions SET revoked_at = ?
  WHERE token_hash <> ? AND revoked_at IS NULL
`);
const revokeAllSessionsStatement = database.prepare(`
  UPDATE browser_sessions SET revoked_at = ? WHERE revoked_at IS NULL
`);
const deleteExpiredSessionsStatement = database.prepare(`
  DELETE FROM browser_sessions WHERE expires_at <= ? OR revoked_at IS NOT NULL
`);
...
export type BrowserSession = { token: string; expiresAt: number };
...
export function isBrowserSessionValid(token: string, now = Date.now()): boolean {
  ...
  const row = readSessionStatement.get(hashSessionToken(token)) as
    ...
    | undefined;
  if (!row || row.revoked_at !== null || row.expires_at <= now) return false;
...
export function revokeOtherBrowserSessions(currentToken: string, now = Date.now()): number {
  if (!currentToken) return 0;
  const result = revokeOtherSessionsStatement.run(now, hashSessionToken(currentToken));
  return Number(result.changes);
}

export function revokeAllBrowserSessions(now = Date.now()): number {
  const result = revokeAllSessionsStatement.run(now);
  return Number(result.changes);
}
